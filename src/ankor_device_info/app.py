"""Desktop hardware monitor with an optional pinned overlay."""
...
# Metric ranges used by progress bars.
# Utilization bars: 0-100%
# Temperature bars: 0-CPU_TEMP_BAR_MAX_C / GPU_TEMP_BAR_MAX_C.
CPU_TEMP_BAR_MAX_C = 120
GPU_TEMP_BAR_MAX_C = 140

# CPU temperature sensors are checked in this order for predictable results.
CPU_TEMP_PREFERRED_SENSORS = ("coretemp", "k10temp", "cpu_thermal", "acpitz")

PSUTIL_READ_EXCEPTIONS = (
    psutil.Error,
    PermissionError,
    FileNotFoundError,
    OSError,
...
class NvidiaMonitor:
    def __init__(self) -> None:
        ...
        self._nvml_errors: tuple[type[BaseException], ...] = (OSError, AttributeError)
        self._initialize()
    ...
    def _initialize(self) -> None:
        ...
        try:
            pynvml.nvmlInit()
            self._nvml = pynvml
            self._nvml_errors = (pynvml.NVMLError, OSError, AttributeError)
            self._handle = pynvml.nvmlDeviceGetHandleByIndex(0)
            self.name = pynvml.nvmlDeviceGetName(self._handle)
            if isinstance(self.name, bytes):
                self.name = self.name.decode("utf-8", errors="replace")
            self.available = True
        except (pynvml.NVMLError, OSError, AttributeError) as exc:
    ...
    def read(self) -> tuple[Optional[float], Optional[float]]:
        ...
        except self._nvml_errors as exc:
...
class SystemSampler:
    def __init__(self) -> None:
        ...
        self._net_baseline_ready = False
        self._cpu_temp_sensor_source: Optional[str] = None
    ...
    def sample(self) -> Snapshot:
        ...
        if self._net_baseline_ready:
            elapsed = now_time - self._last_time
            if elapsed <= 0:
                download_per_sec = 0.0
                upload_per_sec = 0.0
            else:
    ...
    def _read_disk_usage(self):
        ...
        except PSUTIL_READ_EXCEPTIONS as exc:
            LOGGER.debug("Disk usage read failed for '%s': %s", self._disk_mount, exc)
    ...
    def _read_cpu_frequency() -> Optional[float]:
        try:
            ...
            LOGGER.debug("CPU frequency read failed: %s", exc)
            return None

    def _read_cpu_temperature(self) -> Optional[float]:
        if not hasattr(psutil, "sensors_temperatures"):
            self._set_cpu_temp_source(None)
            return None
        ...
        for key in CPU_TEMP_PREFERRED_SENSORS:
            entries = sensor_map.get(key, [])
            values = [entry.current for entry in entries if entry.current is not None]
            if values:
                self._set_cpu_temp_source(key)
                return max(values)

        fallback_source: Optional[str] = None
        fallback_max: Optional[float] = None
        for key, entries in sensor_map.items():
            values = [entry.current for entry in entries if entry.current is not None]
            if not values:
                continue
            current_max = max(values)
            if fallback_max is None or current_max > fallback_max:
                fallback_max = current_max
                fallback_source = key

        self._set_cpu_temp_source(fallback_source)
        return fallback_max

    def _set_cpu_temp_source(self, source: Optional[str]) -> None:
        if source == self._cpu_temp_sensor_source:
            return
        self._cpu_temp_sensor_source = source
        if source is None:
            LOGGER.debug("No CPU temperature sensor source available.")
            return
        LOGGER.debug("CPU temperature sensor source selected: %s", source)
...
class MonitorWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        self.cpu_label, self.cpu_bar = self._metric_card(
            grid,
            0,
            0,
            "CPU Usage",
            with_bar=True,
            tooltip="CPU utilization (expected range: 0-100%).",
        )
        self.cpu_temp_label, self.cpu_temp_bar = self._metric_card(
            ...
            "CPU Temperature",
            with_bar=True,
            bar_max=CPU_TEMP_BAR_MAX_C,
            tooltip=f"CPU temperature in C (expected range: 0-{CPU_TEMP_BAR_MAX_C}).",
        )
        self.ram_label, self.ram_bar = self._metric_card(
            grid,
            1,
            0,
            "RAM Usage",
            with_bar=True,
            tooltip="RAM utilization (expected range: 0-100%).",
        )
        self.swap_label, self.swap_bar = self._metric_card(
            grid,
            1,
            1,
            "Swap Usage",
            with_bar=True,
            tooltip="Swap utilization (expected range: 0-100%).",
        )
        self.disk_label, self.disk_bar = self._metric_card(
            grid,
            2,
            0,
            "Disk Usage",
            with_bar=True,
            tooltip="Disk usage for the monitored mount (expected range: 0-100%).",
        )
        self.gpu_label, self.gpu_bar = self._metric_card(
            grid,
            2,
            1,
            "GPU Usage",
            with_bar=True,
            tooltip="GPU utilization (expected range: 0-100%).",
        )
        self.uptime_label, _ = self._metric_card(
            grid,
            3,
            0,
            "Uptime",
            with_bar=False,
            tooltip="Elapsed time since last system boot.",
        )
        self.gpu_temp_label, self.gpu_temp_bar = self._metric_card(
            grid,
            3,
            1,
            "GPU Temperature",
            with_bar=True,
            bar_max=GPU_TEMP_BAR_MAX_C,
            tooltip=f"GPU temperature in C (expected range: 0-{GPU_TEMP_BAR_MAX_C}).",
        )
        self.network_label, _ = self._metric_card(
            grid,
            4,
            0,
            "Network",
            with_bar=False,
            column_span=2,
            tooltip="Current transfer rate. First sample initializes baseline.",
        )
    ...
    def refresh_metrics(self) -> None:
        ...
        if snapshot.gpu_usage_percent is None:
            ...
            self.gpu_label.setText(f"{gpu_header}: {snapshot.gpu_usage_percent:.1f}%")
            self._set_progress(self.gpu_bar, snapshot.gpu_usage_percent)

        if snapshot.gpu_temp_c is None:
            self.gpu_temp_label.setText("N/A (not reported)")
            self._set_progress(self.gpu_temp_bar, 0.0)
        else:
            self.gpu_temp_label.setText(f"{snapshot.gpu_temp_c:.1f} C")
            self._set_progress(self.gpu_temp_bar, snapshot.gpu_temp_c)
    ...
    def _restore_main_window_geometry(self) -> None:
        ...
        try:
            ...
            LOGGER.debug("Window geometry restore failed: %s", exc)
    ...
    def _set_progress(self, bar: Optional[QProgressBar], value: float) -> None:
        ...
        fill_ratio = (bounded - bar.minimum()) / span
        chunk = self._progress_color(fill_ratio)
    ...
    @staticmethod
    def _progress_color(fill_ratio: float) -> str:
        if fill_ratio >= 0.9:
            return "#f43f5e"
        if fill_ratio >= 0.75:
            return "#f59e0b"
